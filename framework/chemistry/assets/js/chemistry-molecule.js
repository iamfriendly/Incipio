...
//gridslider_end
	...
	$(document).ready(function ()
	{
	    ...
	    $('.' + egs.add_prefix('grid')).each( function()
		{
			$(this).gridSlider
			({
				slider: $(this).hasClass('chemistry-slider'),
		...
		$('.chemistry-back-to-top').click( function()
		{
		...
		var $scrollspy_menu = $('.chemistry-scrollspy a');
		...
		$scrollspy_menu.click( function(e)
		{
			...
			{
				$target.parent().siblings().removeClass('chemistry-current').end().addClass('chemistry-current');
			});
		...
		$(window).scroll( function()
		{
			...
			if (current.length > 0)
			{
				...
				$scrollspy_menu.parent().removeClass('chemistry-current').end().filter('[href=#' + id + ']').parent().addClass('chemistry-current');
			}
		});
		...
		var fix_ie7_grid = function (elem)
		{
			if ( ! elem.hasClass('ie7'))
			{
				elem
					.addClass('ie7')
					.find('.' + egs.add_prefix('col')).each(function ()
					{
						$(this).children().wrapAll('<div class="' + egs.add_prefix('ie7-padding-maker')+ '"></div>');
					});
			}
		};
		...
		var fix_lteie9_grid = function ()
		{
			if ($.browser.msie && $.browser.version < 9)
			{
				...
				$elem.find('.chemistry-col').each(function ()
				{
					$(this).children().eq(-1)
						.addClass('last-child cock');
				});
